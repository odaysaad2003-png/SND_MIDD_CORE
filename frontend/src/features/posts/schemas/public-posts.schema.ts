...
export const publicPostSchema = z
    .object({
        ...
        // The current API shape is string[]. A few deployed legacy records still
        // contain obsolete relative /uploads paths, so media rendering performs
        // a separate safe-URL check without rejecting the whole feed response.
...
export const publicPostResultSchema = z
    .object({
...
export type PublicPostResult = z.infer<typeof publicPostResultSchema>;
